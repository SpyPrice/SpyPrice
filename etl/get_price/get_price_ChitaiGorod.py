...
def _extract_product_info(page):
    try:
        page.wait_for_selector('h1, [class*="price"], [class*="product-price"]', timeout=10000)
    except:
        return None
    ...
    name = name_elem.text_content().strip() if name_elem.count() > 0 else "Неизвестная книга"

    price_elem = page.locator('[class*="price"], .product-price, .current-price, .sale-price').first
...
def get_product_info(url, headless=True):
    ...
    try:
        ...
        if page.locator('#challenge-form').count() > 0:
            page.screenshot(path='chitai_gorod_challenge.png')
            return None

        product_info = _extract_product_info(page)

        if not product_info:
            page.screenshot(path='chitai_gorod_error.png')
            with open('chitai_gorod_debug.html', 'w', encoding='utf-8') as f:
