...
def get_product_info(url, headless=True):
    ...
    try:
        ...
        try:
            ...
            if json_ld_script.count() > 0:
                ...
                if name and price:
                    ...
                    return {"store": "Авто.ру", "name": name, "price_str": price_str, "price_float": price_float, "url": url, "extra": {}}
        except Exception as e:
            print(f"Ошибка парсинга JSON-LD: {e}")

        name_elem = page.locator('h1').first
        name = name_elem.text_content().strip() if name_elem.count() > 0 else "Неизвестный автомобиль"

        price_selectors = ['.CardInfo__price', '[class*="price"]', '.Price__value']
        price_text = None
        ...
        if price_text:
            ...
            if price_clean:
                return {"store": "Авто.ру", "name": name, "price_str": price_text, "price_float": float(price_clean), "url": url, "extra": {}}

        with open(os.path.join(DEBUG_DIR, 'auto_ru_debug.html'), 'w', encoding='utf-8') as f:
            f.write(page.content())
        page.screenshot(path=os.path.join(DEBUG_DIR, 'auto_ru_error.png'))
